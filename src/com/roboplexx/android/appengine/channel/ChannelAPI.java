...
public class ChannelAPI {
   ...
   */
  public ChannelAPI(String listenUrl, String channelKey, HttpClient client, ChannelService channelService) throws IOException, ClientProtocolException {
    this.clientId = null;
//    this.BASE_URL = URL;
...
//  }
   ...
   */
  private String createChannel(String channelKey) throws IOException, ClientProtocolException{
    String token = "";
    //		HttpClient staticClient = new DefaultHttpClient();
    //		HttpGet httpGet = new HttpGet(BASE_URL + "/robo-listen?r=" + key);
    //		HttpGet httpGet = new HttpGet(BASE_URL + "/robots/" + key + "/listen");
    HttpGet httpGet = new HttpGet(listenUrl);
    //		http://localhost:8080/robots/ag1kZXZ-cm9ib3BsZXh4chILEgxSb2JvdFByb2ZpbGUYNQw/subscribe
    try {
      //			XHR xhr = new XHR(staticClient.execute(httpGet));
      XHR xhr = new XHR(httpClient.execute(httpGet));
  ...
  private void longPoll() {
    ...
    this.thPoll = new Thread(new Runnable() {
      ...
      public void run() {
        ...
        System.out.println("Ending channel polling thread");
      }
    });
